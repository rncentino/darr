        ...
        <form id="issueBookForm">
          <div class="mb-3">
            ...
            <select id="studentNo" name="student_id" class="form-select" required>
              ...
              <?php
              $students = $conn->query("SELECT student_id, student_no FROM student");
              while($student = $students->fetch_assoc()) {
                echo "<option value='".$student['student_id']."'>".$student['student_no']."</option>";
              }
              ?>
            </select>
          </div>
          <div class="mb-3">
            ...
            <select id="isbn" name="book_copy_id" class="form-select" required>
                ...
                <?php
                $books = $conn->query("SELECT bc.book_copy_id, bk.isbn FROM bookcopy bc JOIN book bk ON bc.book_id = bk.book_id WHERE bc.status = 'available'");
                ...
                while ($book = $books->fetch_assoc()) {
                    if (!in_array($book['isbn'], $unique_isbns)) { // Check if the ISBN is already added
                        echo "<option value='" . $book['book_copy_id'] . "'>" . $book['isbn'] . "</option>";
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-success" id="confirmReturn">Yes</button>
      </div>
    </div>
  </div>
</div>
 ...
 if ($result->num_rows > 0) {
     while($row = $result->fetch_assoc()) {
         ...
         if(empty($row['date_returned'])) {
             echo "<td><button class='btn btn-secondary return-book' data-id='".$row['borrow_id']."' data-bs-toggle='modal' data-bs-target='#returnBookModal'>Return</button></td>";
         } else {
             echo "<td><button class='btn btn-success return-book' data-id='".$row['borrow_id']."' data-bs-toggle='modal' data-bs-target='#returnBookModal' disabled>Returned</button></td>";
         }
